...
with open("dececco.csv", "a", encoding="utf-8", newline="") as file:
    ...
    for ii, c in enumerate(colonna):
        ...
        if len(parti) == 2:
            ...
            cognome = " ".join(parti[:2])  # Unisce le prime due parti
            nome = parti[2]
    ...
    # Recupera gli articoli pubblicati dall'autore
        ...
        for jj, title in enumerate(articles):
            ...
            authors = functions.get_authors_by_eid(article_eid, API_KEY)
    ...
    #        year = fun.get_publication_year(article_doi)
            ...
            year = functions.get_article_year_by_eid(article_eid, API_KEY)
